...
export default function FacilitiesPage() {
  ...
  const fetchFacilities = async (lat, lng) => {
    ...
    try {
      const response = await HospitalService.findNearbyHospitals(
        lat, lng, 30, filterType || null, filterEmergency ? true : null
      );
  ...
  }, [location, filterType, filterEmergency]);

  return (
    <div className="flex flex-col h-full bg-gray-50 p-4 max-w-7xl mx-auto">
      <h1 className="text-2xl font-bold text-gray-800 mb-4">Find Healthcare Facilities</h1>
        ...
        <div className="bg-yellow-50 border-l-4 border-yellow-400 p-4 mb-4 rounded shadow-sm">
          <div className="flex items-center">
            ...
            <p className="text-sm text-yellow-700">
              You are offline. Showing cached facilities nearby.
            </p>
          </div>
        </div>
      )}
      ...
      {!location && !locLoading && (
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200 mb-6 flex flex-col items-center text-center">
          ...
          <p className="text-gray-600 mb-4">Please share your location to find nearby hospitals and clinics.</p>
          <button 
      ...
      {locLoading && (
        <div className="flex justify-center p-8 text-gray-500">Getting location...</div>
      )}
          ...
          <div className="w-full lg:w-1/3 flex flex-col gap-4 overflow-hidden">
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 flex-1 overflow-y-auto">
              <div className="p-4 border-b border-gray-100 flex justify-between items-center bg-gray-50 sticky top-0">
                ...
                {dataSource === 'offline' && <span className="text-xs bg-gray-200 px-2 py-1 rounded text-gray-700">Cached</span>}
              </div>
              ...
              {!loading && facilities.length === 0 && (
                <div className="p-8 text-center text-gray-500">
                  No facilities found nearby. Try expanding your search or removing filters.
                </div>
              )}
                ...
                {facilities.map((f) => (
                  <div key={f.id} className="p-4 hover:bg-gray-50 transition">
                    <div className="flex justify-between items-start mb-1">
                      <Link href={`/facilities/${f.id}`} className="font-bold text-blue-700 hover:underline">
                        {f.name}
                      </Link>
                      {f.distance_km && <span className="text-xs font-semibold text-gray-500 bg-gray-100 px-2 py-1 rounded-full">{f.distance_km} km</span>}
                    </div>
                    <p className="text-sm text-gray-600 mb-2">{f.facility_type}</p>
                    ...
                    <div className="flex gap-2 mt-2">
                      <a 
                        href={HospitalService.getDirectionsLink(f.latitude, f.longitude)}
                        target="_blank"
